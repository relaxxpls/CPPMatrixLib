...
void precomp(){
    ...
    for(ll i=2;i<200001;i++){
        ...
        fact[i]=i*fact[i-1];
        fact[i]%=M2;
        inv[i]=binex(fact[i],M2-2,M2);
    }
}

ll C(ll n, ll r){

    return ((fact[n]*inv[r])%M2*inv[n-r])%M2;
}

ll path(pll &a, pll &b){

    if(b.ss-a.ss<0||b.ff-a.ff<0){

        return 0;
    }

    return C(b.ss-a.ss+b.ff-a.ff,b.ff-a.ff);
}
...
void solve(){
    ...
    for(ll i=1;i<=n+1;i++){
        ...
        for(ll j=1;j<i;j++){
            ...
            dp[i]+=M2;
            dp[i]-=((dp[j]*path(a[j],a[i]))%M2);
            dp[i]%=M2;
            //cout<<"Temp: "<<path(a[j],a[i])<<"\n";
        }
    }
