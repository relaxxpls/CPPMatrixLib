...
void solve(){
    ...
    for(ll i=2;i<(1<<n);i++){
        ...
        for(ll k=0;k<temp.sz;k++){

            for(ll l=k+1;l<temp.sz;l++){
            
                ans0+=a[temp[k]+1][temp[l]+1];
            }
        }

        dp[i]=ans0;

        for(ll j=1;j<(1<<temp.sz);j++){

            if(__builtin_popcount(j)>temp.sz){
