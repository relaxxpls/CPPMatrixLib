...
void update(vll &BITree, ll pos, ll val){
    ...
    while(pos<BITree.sz){
        ...
        BITree[pos]=max(BITree[pos],val);
        
        pos+=(pos&-pos);
    }
}

ll max(vll &BITree, ll pos){

    ll maxi=0;

    while(pos>0){

        maxi=max(maxi,BITree[pos]);
...
void solve(){
    ...
    update(BITree,h[1],a[1]);

    for(ll i=2;i<=n;i++){

        ll temp=(max(BITree,h[i]-1))+a[i];

        update(BITree,h[i],temp);
    }
