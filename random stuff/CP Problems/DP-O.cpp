...
ll finder(ll men, ll women, vvll &a, vvll &sto){
    
    if(sto[men][women]!=-1){

        return sto[men][women];
    }

    if(__builtin_popcount(women)!=men){

        return sto[men][women]=0;
    }
    ...
    while((women>>q)>0){
        ...
        if((women&(1<<q))&&(a[men][q+1])){

            sum+=finder(men-1,(women^(1<<q)),a,sto);
            sum%=M2;
        }
        q++;
    }

    return sto[men][women]=sum;
}
...
void solve(){
    ...
    vvll sto(n+1,vll(1<<n,-1));

    sto[0][0]=1;

    ll women;

    cout<<finder(n,((1<<n)-1),a,sto)<<"\n";
